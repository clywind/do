...
function callbackFunc(options, _oldData, data, isSucceed){
		...
		if (_fUrl.indexOf(_mock.url) ==0 && _mock.result && _mock.type==options.type){
			...
			if (_mock.isSuccess){
				do_InitData.readFile(_mockFile, function(data) {
					callbackFunc(options, _oldData, data, true);
				});			
			}
			if (!_mock.isSuccess){
				do_InitData.readFile(_mockFile, function(data) {
					callbackFunc(options, _oldData, data, false);
				});
			}
			return true;
		}
	...
	if (d.cacheLastResult){
		...
		if (do_Storage.fileExist(_cacheFile)){
			do_Storage.readFile(_cacheFile, function(data) {
				...
				if (d.isSuccess) d.success.call(this, data);
 ...
 * @param options 传入的选项内容(也可以是自定义的选项名称)
		//上级选项名称（可继承选项内容）
   		parent:null,
		//服务请求的根路径
		rootUrl:"",
